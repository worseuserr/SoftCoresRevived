...
float Temp::GetTimeOfDayModifier()
{
	if (CLOCK::GET_CLOCK_HOURS() >= 5 && CLOCK::GET_CLOCK_HOURS() < 11)
		return 1.5f;
	if (CLOCK::GET_CLOCK_HOURS() >= 11 && CLOCK::GET_CLOCK_HOURS() < 14)
		return 1.25f;
	if (CLOCK::GET_CLOCK_HOURS() >= 14 && CLOCK::GET_CLOCK_HOURS() < 18)
		return 1.75f;
...
float Temp::GetClothingTemperaturePointsRequired()
{
	...
	if (Plr::GetSurroundingTemperature() < temperaturePoints[0]) // coldest
		return 9.0f;
	if (temp >= temperaturePoints[0] && temp < temperaturePoints[1])
		return 8.5f;
	if (temp >= temperaturePoints[1] && temp < temperaturePoints[2])
		return 8.0f;
	if (temp >= temperaturePoints[2] && temp < temperaturePoints[3])
		return 7.5f;
	if (temp >= temperaturePoints[3] && temp < temperaturePoints[4])
		return 7.0f;
	if (temp >= temperaturePoints[4] && temp < temperaturePoints[5])
		return 6.5f;
	if (temp >= temperaturePoints[5] && temp < temperaturePoints[6])
		return 6.0f;
	if (temp >= temperaturePoints[6] && temp < temperaturePoints[7])
		return 5.5f;
	if (temp >= temperaturePoints[7] && temp < temperaturePoints[8])
		return 5.0f;
	if (temp >= temperaturePoints[8] && temp < temperaturePoints[9])
		return 4.5f;
	if (temp >= temperaturePoints[9] && temp < temperaturePoints[10])
		return 4.0f;
	if (temp >= temperaturePoints[10] && temp < temperaturePoints[11])
		return 3.5f;
	if (temp >= temperaturePoints[11] && temp < temperaturePoints[12])
		return 3.0f;
	if (temp >= temperaturePoints[12]) // hottest
		return 2.5f;
	return 0.0f;
}
